...
void vCopyBluetoothBuffer( uint8_t* uTempWifiBuffer )
{
	vCopyWifiBuffer( uTempWifiBuffer ); 			                                    
}
...
uint8_t uBluetoothScanDis( uint8_t uDuration,uint8_t uResponses )
{
	...
	if( EXE_SUCCEED==uReportWifiSemErr( WAIT_SEM_1_TIME,(uint8_t *)"Bluetooth BTSTARTDISC Err" ) ){
		puTempArr = calloc( 1,sizeof(g_Usart2_Rx_Buf) );
		if( NULL==puTempArr ){
			return EXE_FAILED;
		}
...
{	
	...
	return uReportWifiSemErr( WAIT_SEM_4_TIME,(uint8_t *)"Bluetooth BTENCDEV Err" );
...
uint8_t uBluetoothSendDatas( uint8_t* upData,uint32_t uLen )
{
	...
	return uReportWifiSemErr( WAIT_SEM_TIME,(uint8_t *)"Bluetooth Datas Err" );
}
