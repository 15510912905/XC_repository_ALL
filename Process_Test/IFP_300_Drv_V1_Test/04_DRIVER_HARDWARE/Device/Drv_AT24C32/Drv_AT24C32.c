...
*/
DRV_AT24C32_TYPE* At24c32Init(DRV_AT24C32_ENUM id, uint8_t addr,\
...
{	
	...
	p_unit->id             = id;				/*模块id*/
	p_unit->addr           = addr;
	/* 软件IIC上升沿时间不够 有TVS管 需要降低速率 */
	p_unit->iic_port = SimulateIicInit(	SIMULATE_IIC_1, 14, \
										sda_port_number, sda_pin_number, \
										scl_port_number, scl_pin_number);	/*引脚端口号 速率默认为4 */	
	
//	p_unit->iic_port = SimulateIicInit(	SIMULATE_IIC_1, 0, \
//									    sda_port_number, sda_pin_number, \
//									    scl_port_number, scl_pin_number);	/*引脚端口号 速率默认为4 */
...
uint8_t At24c32ReadByte(DRV_AT24C32_TYPE* p_unit, uint16_t address)
{
	...
	Drv_SimulateIicSendByte(p_unit->iic_port, p_unit->addr);			/*发送写命令0xA0*/
	...
	Drv_SimulateIicSendByte(p_unit->iic_port, p_unit->addr+1);			/*发送读命令0xA1*/
	if(Drv_SimulateIicWaitAck(p_unit->iic_port) == false)		/*等待ACK信号*/
	{
		Drv_SimulateIicStop(p_unit->iic_port);					/*发送stop信号*/
		return false;
...
uint16_t At24c32ReadTwoByte(DRV_AT24C32_TYPE* p_unit, uint16_t address)
{
	...
	Drv_SimulateIicSendByte(p_unit->iic_port, p_unit->addr+1);			/*发送读命令0xA0*/
	if(Drv_SimulateIicWaitAck(p_unit->iic_port) == false)		/*等待ACK信号*/
	{
		Drv_SimulateIicStop(p_unit->iic_port);					/*发送stop信号*/
		return false;
