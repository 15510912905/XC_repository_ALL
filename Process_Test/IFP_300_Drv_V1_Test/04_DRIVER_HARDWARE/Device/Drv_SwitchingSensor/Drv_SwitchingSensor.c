...
void Drv_AllSwitchingSensorDebounce_Loop(void)
{
	...
	while(1)
	{
		...
		for(i = 0; i < SWITCHINGSENSOR_BUTT; i++)
		{
			...
			{
				buffer_now = BSP_ReadPin(_gp_SwitchingSensorUnits[i]->port_number, _gp_SwitchingSensorUnits[i]->pin_number);	/*读取当前IO状态*/
				if(_gp_SwitchingSensorUnits[i]->delay_flag == 0)	/*没在消抖*/
				{
					if(buffer_now != _gp_SwitchingSensorUnits[i]->buffer)	/*当前IO状态与之前记录不一致的，开启消抖*/
					{
						_gp_SwitchingSensorUnits[i]->buffer_check = buffer_now;	/*记录消抖状态*/
						_gp_SwitchingSensorUnits[i]->delay_count = _gp_SwitchingSensorUnits[i]->delay_time;	/*记录消抖时间*/
						_gp_SwitchingSensorUnits[i]->delay_flag = 1;	/*开启消抖*/
						if(_gp_SwitchingSensorUnits[i]->delay_count <= 0)	/*如果不需计时*/
						{
							_gp_SwitchingSensorUnits[i]->buffer = _gp_SwitchingSensorUnits[i]->buffer_check;
							if((_gp_SwitchingSensorUnits[i]->buffer == 0) && (_gp_SwitchingSensorUnits[i]->response_type == FALLING))	/*下降沿发送信号量*/
							{
								if(_gp_SwitchingSensorUnits[i]->sem_switch == EN_SEM)	/*信号量开关开启是，才发送信号量*/
									OSSemPost(_gp_SwitchingSensorUnits[i]->sem);
							}
							else if((_gp_SwitchingSensorUnits[i]->buffer == 1) && (_gp_SwitchingSensorUnits[i]->response_type == RISING)) /*上升沿发送信号量*/
							{
								if(_gp_SwitchingSensorUnits[i]->sem_switch == EN_SEM)
									OSSemPost(_gp_SwitchingSensorUnits[i]->sem);
							}
							_gp_SwitchingSensorUnits[i]->delay_flag = 0;	/*停止消抖*/	
						}
					}
				}
				else												/*在消抖*/
				{
					if(_gp_SwitchingSensorUnits[i]->delay_count > 0)	/*消抖计时中*/
					{
						_gp_SwitchingSensorUnits[i]->delay_count--;
					}
					else	/*计时结束*/
					{
						if(buffer_now == _gp_SwitchingSensorUnits[i]->buffer_check)	/*当前IO状态与消抖前记录的一致，则更新为buffer值，并发送信号量，否则不更新*/
						{
							_gp_SwitchingSensorUnits[i]->buffer = _gp_SwitchingSensorUnits[i]->buffer_check;
							if((_gp_SwitchingSensorUnits[i]->buffer == 0) && (_gp_SwitchingSensorUnits[i]->response_type == FALLING))	/*下降沿发送信号量*/
							{
