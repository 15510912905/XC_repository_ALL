						...
						BSP_PORT_ENUM step_port_number,    BSP_PIN_ENUM step_pin_number)
{
	...
	if(p_mcc->sem == NULL)						/*组件信号量*/
		OSSemCreate( p_mcc->sem,"p_mccSem",1,(OS_ERR *)&err );
...
/* 单个组件复位
  ...
  返回值：err, OS报错代码
*/
uint8_t MCC_Reset(DRV_MCC_TYPE* p_mcc, uint32_t timeout)
{
	uint8_t err = OS_ERR_NONE;
	...
	if((p_mcc->reset_status) != BSP_ReadPin(p_mcc->reset_port_number, p_mcc->reset_pin_number))	/*如果光耦没有在复位状态，则寻找复位状态*/
    {
		...
		else						/*当前坐标不为0，快速接近*/
		{
			...
			else
			{
				err = _MCC_Move_SubdivisionStep(p_mcc, \
												p_mcc->reset_dir, \
												abs(p_mcc->position) - (p_mcc->compensation->reset_compensation * p_mcc->subdivision), \
												timeout, \
												MCC_MOVE_RESET, \
												0, \
												MCC_LINE_AUTO);
	...
	else																						/*如果光耦在复位状态，先退出，再复位*/
	{
		...
		{
			OSTimeDlyHMSM(0, 0, 0, 200,OS_OPT_TIME_HMSM_STRICT,( OS_ERR *)&err);	/*延时200ms避免抖动*/
			p_mcc->position = 0;
			err = _MCC_Move_SubdivisionStep(p_mcc, \
											p_mcc->reset_dir, \
											p_mcc->max_step, \
...
bool MCC_SetStempMotorLine(DRV_MCC_TYPE* p_mcc, uint32_t start_fre,	uint32_t end_fre,	uint32_t ech_lader_step, uint32_t up_max_lader,	uint32_t s_par, uint8_t ch)
{
	switch(ch)
	{
		case 0:
			...
			CalcMotorFre(p_mcc->motor->subdivision, p_mcc->motor->line_high, p_mcc->motor->line_hig_fre);	/*高速运动频率表计算*/
			break;
		...
		case 1:
			...
			CalcMotorFre(p_mcc->motor->subdivision, p_mcc->motor->line_low, p_mcc->motor->line_low_fre);	/*低速运动频率表计算*/
			break;
	}
					  ...
					  MCC_LINE_CHOICE_ENUM line_choice)
{
	...
	/*设置慢速运动脉冲数*/
	if(status == MCC_MOVE_DISTANCE)	/*走固定脉冲，不需要慢速运动*/
	{
		slow_step = 0;
	}	
	...
	switch(line_choice)
	{
		case MCC_LINE_AUTO:
			/*按总脉冲数调用不同运动曲线*/
			if(step >= (p_mcc->motor->line_high->ech_lader_step * p_mcc->motor->line_high->up_max_lader * 2))	/*p_mcc->motor->subdivision * */
			{
				Motor_MoveSP_Begin(p_mcc->motor, \
								   dir, \
								   step, \
								   slow_step, \
								   1);									/*大于高速运动曲线加减速脉冲总数的按高速曲线运动*/
			}
			else if(step >= (p_mcc->motor->line_low->ech_lader_step * p_mcc->motor->line_low->up_max_lader * 2))	/*p_mcc->motor->subdivision * */
			{
				Motor_MoveSP_Begin(p_mcc->motor, \
								   dir, \
								   step, \
								   slow_step, \
								   0);									/*大于低速运动曲线加减速脉冲总数的按低速曲线运动*/
			}
			else
			{
				Motor_MoveRL_Begin(p_mcc->motor, \
								   ...
								   step, \
								   slow_step, \
								   (p_mcc->motor->line_low_fre[0] < p_mcc->motor->line_hig_fre[0]) ? p_mcc->motor->line_low_fre[0] :p_mcc->motor->line_hig_fre[0]);	/*否则按匀速运动*/
			}
			break;
		case MCC_LINE_SLOW:
			Motor_MoveRL_Begin(p_mcc->motor, \
	...
	/*等待运动完成*/
	OSSemPend(p_mcc->sem, timeout,OS_OPT_PEND_BLOCKING,NULL,( OS_ERR *)&err);
	
	p_mcc->status = MCC_IDLE;	/*恢复空闲状态*/
	
	if(err != OS_ERR_NONE)	/*运动出错，电机停止*/
	{
...
{
	if((p_unit->status != MCC_IDLE) && (p_unit->status != MCC_MOVE_RESET))		/*MCC在动，并且不是复位*/
	{
		Motor_Move_Stop(p_unit->motor);		/*停止电机动作*/
...
void __Motor_Done(DRV_STEPMOTOR_TYPE* motor)
{
	...
	{
		case MOTOR_1:
			OSSemPost(_gp_MCCunits[MCC_1]->sem,OS_OPT_POST_1,(OS_ERR  *)&err);
			break;
		case MOTOR_2:
			OSSemPost(_gp_MCCunits[MCC_2]->sem,OS_OPT_POST_1,(OS_ERR  *)&err);
			break;
		case MOTOR_3:
			OSSemPost(_gp_MCCunits[MCC_3]->sem,OS_OPT_POST_1,(OS_ERR  *)&err);
			break;
		case MOTOR_4:
			OSSemPost(_gp_MCCunits[MCC_4]->sem,OS_OPT_POST_1,(OS_ERR  *)&err);
			break;
		default:
			break;
	}
