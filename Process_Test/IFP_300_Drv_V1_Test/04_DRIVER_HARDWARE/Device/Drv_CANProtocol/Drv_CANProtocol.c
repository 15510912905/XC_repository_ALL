...
//  返回值：true成功，false失败
//*/
//bool Drv_CANProtocol_SendErrorToBuffer(uint8_t can_id, uint8_t serial_number, uint16_t cmd, uint32_t frame_id, uint32_t err_code)
//{
...
///* CAN发送一个执行成功信息加一个数据到发送缓存
//  --------------------------------- OK
//  写入缓冲区，等待发送
//  入口：CAN ID，流水号，命令字，firm id
//  返回值：true成功，false失败
//*/
//bool Drv_CANProtocol_SendDoneAndDataToBuffer(uint8_t can_id, uint8_t serial_number, uint16_t cmd, uint32_t frame_id, uint32_t data)
//{
//	DRV_CAN_RETURN_DATA_TYPE data_out = {0};
//	uint8_t os_err = OS_ERR_NONE;
//	bool err = true;
//	uint8_t count = 0;
//			
...
//	do {
//		count++;
//		err = _Drv_CANProtocol_SendToBuffer(can_id, (uint8_t *)(&data_out), sizeof(DRV_CAN_RETURN_DATA_TYPE), MARK_END);
//		if(err != false)
//			break;
//		else
//		{
...
///* CAN发送一个执行成功信息加4个数据到发送缓存
//  --------------------------------- OK
//  写入缓冲区，等待发送
//  入口：CAN ID，流水号，命令字，firm id
//  返回值：true成功，false失败
//*/
//bool Drv_CANProtocol_SendDoneAnd4DatasToBuffer(uint8_t can_id, uint8_t serial_number, uint16_t cmd, uint32_t frame_id, uint32_t* datas)
//{
//	DRV_CAN_RETURN_4_DATAS_TYPE data_out = {0};
//	uint8_t os_err = OS_ERR_NONE;
//	bool err = true;
//	uint8_t count = 0;
//			
...
//	data_out.cmd = cmd;
//	data_out.status = CAN_STATUS_WORD_DONE;
//	data_out.data1 = datas[0];
//	data_out.data2 = datas[1];
//	data_out.data3 = datas[2];
//	data_out.data4 = datas[3];
//	
//	OSMutexPend(_gp_CANunit->send_to_buffer_mutex, 100, &os_err);
//	if(os_err != OS_ERR_NONE)
//	{
...
//	do {
//		count++;
//		err = _Drv_CANProtocol_SendToBuffer(can_id, (uint8_t *)(&data_out), sizeof(DRV_CAN_RETURN_4_DATAS_TYPE), MARK_END);
//		if(err != false)
//			break;
//		else
//		{
...
///* CAN发送一个执行成功信息加5个数据到发送缓存
//  --------------------------------- OK
//  写入缓冲区，等待发送
//  入口：CAN ID，流水号，命令字，firm id
//  返回值：true成功，false失败
//*/
//bool Drv_CANProtocol_SendDoneAnd5DatasToBuffer(uint8_t can_id, uint8_t serial_number, uint16_t cmd, uint32_t frame_id, uint32_t* datas)
//{
//	DRV_CAN_RETURN_5_DATAS_TYPE data_out = {0};
//	uint8_t os_err = OS_ERR_NONE;
//	bool err = true;
//	uint8_t count = 0;
//			
...
//	do {
//		count++;
//		err = _Drv_CANProtocol_SendToBuffer(can_id, (uint8_t *)(&data_out), sizeof(DRV_CAN_RETURN_5_DATAS_TYPE), MARK_END);
//		if(err != false)
//			break;
//		else
//		{
...
//											0xFFFFFFFF, 10004);		/*报错，接收ID为0或流水号为0*/
