...
//			sprintf( (char *)strff+RecIndex,"%d%c%s",l_iRecTemp,'%'," " );                 /* 百分比显示 */
		}else{                                                                            
//			g_uElectricValue = VOLTAGE_THRESHOLD_PROMPT;                                   /* 全局查看电量 */
//			l_uRecDraw = ELECTRIC_4_VALUE;                                                
//			sprintf( (char *)strff+RecIndex,"%d%c%s",5,'%'," " );                          /* 错误情况不显示 */
		}			
...
#endif					
						if( ELECTRIC_4_VALUE!=l_uRecDraw ){
