...
uint8_t uConfigAllFileInit( void )
{
	...
	if( 0xffff==gp_stuConfigFile->uLanReferValue){
		gp_stuConfigFile->uLanReferValue=1;
	}
	...
	if( 0xff==gp_stuConfigFile->uCutEnCu ){
		gp_stuConfigFile->uCutEnCu = CH_ON;//EN_ON;
	}
...
uint8_t uTestReadIdItem( void )
{
	...
	if( EXE_SUCCEED==uRecError ){
		uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)g_IdMemory.project_name );
	}else{
		uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"ID Error" );
	}
...
{			
	uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"Buzzer " );
	uBuzzerTimes( BUZZER_ONE_TIME );	
...
{		
	...
	gp_stuConfigFile->uLedOnOff = gp_stuConfigFile->uLedOnOff%2;
	if( FLUORESCENCE_ON==gp_stuConfigFile->uLedOnOff ){	
		uFluorescenceLight( FLUORESCENCE_ON );            /* 开启LED	 */
		
		uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"Led Open" );		
	}else if( FLUORESCENCE_OFF==gp_stuConfigFile->uLedOnOff ){		
		uFluorescenceLight( FLUORESCENCE_OFF );           /* 关闭LED	 */
		
		uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"Led Close" );					
	}else{
		uFluorescenceLight( FLUORESCENCE_OFF );           /* 关闭LED	 */
...
int16_t iRealElectricValue( uint8_t uAverage )
{
	...
	for( i=0;i<uAverage;i++ ){
		iRecTemp += BSP_ReadAdc( VSENSE_PORT,VSENSE_PIN );       /* 电量AD采集 */
	}
	iRecTemp = iRecTemp/uAverage;
...
//	iRecTemp = (((iRecTemp*VOLTAGE_COEF)/VOLTAGE_1_REF)-VOLTAGE_THRESHOLD);
//	if( BSP_ReadPin(CHARGE_STATUS_PORT,CHARGE_STATUS_PIN) ){     /* 使用电池时 */
//		g_uChargeStauts = DISCHARGE_STATE;
////		iRecTemp = (((iRecTemp*VOLTAGE_COEF)/VOLTAGE_1_REF)-VOLTAGE_THRESHOLD);  //(uRecTemp-1360)/2.8;	                        /* 电量算法 */
//	}else{                                                       /* 充电时 */
//		g_uChargeStauts = CHARGE_STATE;
////		iRecTemp = (((iRecTemp*VOLTAGE_1_COEF)/VOLTAGE_1_REF)-VOLTAGE_1_THRESHOLD);  //(uRecTemp-1360)/2.8;	                    /* 电量算法 */
//	}	
//	
//#if DEBUH_UART1	
//	#if PRIVATE_ELECTRIC_UART1
...
uint8_t uInitIdInfo( ID_Info* stuIdInfo )
{
	...
	if( (EXE_FAILED==uVerifyId( stuIdInfo )) ){                                  /* 效验ID卡 */		
		...
		if( EXE_SUCCEED!=uRecBarStrage ){
			...
			if( EN_ON==gp_stuConfigFile->uCutEnCu ){
				uPrompterWarn( (uint8_t *)"CoverID?" );                                /* 提示用户 让用户选择 */
			}else if( CH_ON==gp_stuConfigFile->uCutEnCu ){
...
uint8_t DisplayReferenceData( uint8_t pageid )
{
	...
	if( BUTTON_VALUE_REFERENCE_DOWN==pageid ){
		...
		if( g_projectSerial.ureference_count-1>=0 ){
