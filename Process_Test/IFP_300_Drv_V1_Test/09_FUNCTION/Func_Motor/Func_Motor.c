...
void vTestMotorBackward( void )
{
	OS_ERR err = OS_ERR_NONE;
	
	uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"The motor starts to move" );
	MotorForward();
	OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);                //延时1min		
	MotorBackward();
	OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);                //延时1min			
	uTjc4832tSetLableValue( TJC_SPITEST_PAGE,SPI_TEST_DIS_CON_ID,(uint8_t *)"End of motor movement" );
}
...
void MotorForward( void )
{
	...
	PWM_Config(_gp_PWM1, 3200, MOTOR_FREQUENCY);    /* 配置PWM方波 */	
		...
		do
		{
			...
			{	
				...
				if( !BSP_ReadPin(MOTOR_RESET_PORT,MOTOR_RESET_PIN) )
				{
					PWM_Disable( _gp_PWM1 );	/*停止当前运动*/;
					/* 此时再走10步防止临界问题 */
					PWM_Config( _gp_PWM1,6*MOTOR_PULSE,MOTOR_FREQUENCY );    /* 配置PWM方波 300起 0.13mm */			
					PWM_Enable( _gp_PWM1 );	          	                     /*使能PWM方波 电机走 */					
					if( UI_Y_PROMPT==uPrompt ){
