...
uint8_t uOrderAnalysis( uint8_t uOrder,uint8_t *puOrdArr,uint8_t uSize )
{
	switch( uOrder ){	
		...
		case TJC_INVALID_ORDER:
			uTjcErrorEvent( TJC_INVALID_ORDER );
			break;
...
uint8_t uPageButton( uint8_t uPage,uint8_t pButton )
{
	...
	switch( uPage ){
		...
		case TJC_SPITEST_PAGE:
			return uSpiTestButton( pButton );
		case TJC_LOGIN_PAGE:
...
uint8_t uSpiTestButton( uint8_t pButton )
{
	switch( pButton ){
		case 4:			
			return BUTTON_VALUE_BUZZER;
		case 6:			
		...
		case 8:
			return ID_PAGE;
		case 9:	
			return BUTTON_VALUE_LED_CTRL;
		case 10:
			return BUTTON_VALUE_MOTOR_DOWN;
		case 11:
			return BUTTON_VALUE_SHUTDOWN;
...
uint8_t uReplyStrRegisterButton( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){
		...
		case 4:	
#if LOGIN_CODE
			puTempArr = calloc( USER_NAME_SIZE+1,sizeof(uint8_t) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
{	
	...
	switch( pButton ){
		...
		case 5:	
			puTempArr = calloc( 1,sizeof(USER_MORE_INFO) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
uint8_t uReplyStrServerConfig( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){
		...
		case 3:		
			puTempArr = calloc( sizeof(g_ServerIPAddr)+sizeof(g_ServerPort),sizeof(uint8_t) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
uint8_t uReplyStrButton( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){
		...
		case 3:		
			puTempArr = calloc( 1,sizeof(USER_TIME_FILE) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
uint8_t uReplyStrConfigButton( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){
		...
		case 3:	
			puTempArr = calloc( 1,sizeof(USER_INPUT_CONFIG_FILE) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
uint8_t uReplyStrSystemButton( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){
		...
		case 3:	
			puTempArr = calloc( 1,sizeof(USER_INPUT_CONFIG_FILE)+sizeof(g_uInputPrintTitles) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
...
uint8_t uReplyStrReferenceButton( uint8_t pButton,uint8_t *puOrdArr,uint8_t uSize )
{
	...
	switch( pButton ){		
		case 3:	
			puTempArr = calloc( 1,sizeof(USER_REFERRENCE) ); /* 申请内存 */
			if( NULL==puTempArr ){
				return EXE_FAILED;
			}
		...
		case 2:
			...
			if( uSize<128 ){
				if( EN_ON==gp_stuConfigFile->uCutEnCu ){
					...
					}else if( NULL==strncmp( (const char *)"CoverID?",(const char *)puOrdArr,uSize ) ){
		...
		case 5:	
			...
			if( uSize<128 ){
				if( EN_ON==gp_stuConfigFile->uCutEnCu ){
					...
					}else if( NULL==strncmp( (const char *)"ClearHistory?",(const char *)puOrdArr,uSize) ){
		...
		case 2:	
			...
			if( uSize<128 ){
				if( EN_ON==gp_stuConfigFile->uCutEnCu ){
					...
					}else if( NULL==strncmp( (const char *)"Nonet",(const char *)puOrdArr,uSize ) ){
						uRecTemp = BUTTON_VALUE_LIS_CANCLE;
					}
				}else{
					if( NULL!=strstr((const char *)puOrdArr,(const char *)"second") ){
...
uint8_t uTjcAsciiToInt( uint8_t* msgTemp,uint32_t * pu32RecTemp,uint8_t strSize )
{
