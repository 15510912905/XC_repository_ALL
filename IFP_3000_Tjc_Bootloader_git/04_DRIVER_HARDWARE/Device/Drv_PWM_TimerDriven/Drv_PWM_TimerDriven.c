...
*/
bool PWM_Config(DRV_PWM_TIMERDRIVEN_TYPE* p_unit, uint32_t step, uint32_t fre)
{
...
{	
	if(p_unit->status == PWM_READY)
	{
		...
		BSP_WritePin(MOTOR_EN_PORT,MOTOR_EN_PIN,MOTOR_EN_FALSE);    /* 低电平使能 高电平失能  */
...
uint32_t PWM_Disable(DRV_PWM_TIMERDRIVEN_TYPE* p_unit)
{
	if(p_unit->status == PWM_BUSY)		/*该模块在运行时才进行停止操作*/
	{
		...
		BSP_WritePin(MOTOR_EN_PORT,MOTOR_EN_PIN,MOTOR_EN_TRUE);    /* 低电平使能 高电平失能  */
