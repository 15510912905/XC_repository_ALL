...
DRV_HARDAD7685_TYPE* _gp_HardAd7685Units[HARDAD7685_BUTT] = {0};	/*AD7685模数转换模块的注册表*/
...
*/
DRV_HARDAD7685_TYPE* Drv_HardAd7685Init(DRV_HARDAD7685_ENUM id, BSP_SPI_ENUM spi_number, \
...
{	
	DRV_HARDAD7685_TYPE* p_unit = (DRV_HARDAD7685_TYPE*)calloc(1, sizeof(DRV_HARDAD7685_TYPE));	/*申请内存*/
	...
	/*如果当前模块存在，取消分配新存储区*/
	if(_gp_HardAd7685Units[id] != NULL)
	{
		free(p_unit);
		p_unit = _gp_HardAd7685Units[id];
	}
	else
	{
		_gp_HardAd7685Units[id] = p_unit;
	}
	...
	BSP_InitSpiMaster16Bit(spi_number, \
	...
	/*-------------------------------变量*/
	p_unit->status     = HARDAD7685_READY;
	p_unit->tx_buffer  = 0;
	p_unit->rx_buffer  = 0;
...
*/
uint16_t HardAd7685OneAcq(DRV_HARDAD7685_TYPE* p_unit)
{
    //uint16_t data_out = 0;
    
    _HardAd7685Start(p_unit);
    _HardAd7685Wait(p_unit);
    p_unit->rx_buffer = _HardAd7685ReadData(p_unit) ;
...
/* AD7685启动
  ...
  返回值：true 成功，false 失败
*/
static bool _HardAd7685Start(DRV_HARDAD7685_TYPE* p_unit)
{
	BSP_SpiNssSet(p_unit->nss_port_number, p_unit->nss_pin_number, NSS_LOW);	/*CNV拉低*/
	_HardAd7685DelayUs(p_unit, 1);
...
/* AD7685等待转换
  ...
  返回值：true 成功，false 失败
*/
static bool _HardAd7685Wait(DRV_HARDAD7685_TYPE* p_unit)
{
	_HardAd7685DelayUs(p_unit, 5);	/*等待5us*/
	return true;
} 
...
/*  AD7685模块延时n微秒
  ...
  返回值：true 成功，false 失败
*/
static bool _HardAd7685DelayUs(DRV_HARDAD7685_TYPE* p_unit, uint32_t time)
{    
   unsigned short i = 0; 
...
/*  AD7685模块读取AD值
  ...
  返回值：true 成功，false 失败
*/
static uint16_t _HardAd7685ReadData(DRV_HARDAD7685_TYPE* p_unit)
{
	uint16_t data_out = 0;
