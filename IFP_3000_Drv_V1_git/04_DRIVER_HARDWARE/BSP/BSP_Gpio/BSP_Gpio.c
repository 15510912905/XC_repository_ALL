...
bool BSP_Init_PinInterruptIpu(BSP_PORT_ENUM port_number, BSP_PIN_ENUM pin_number)
{
	GPIO_InitTypeDef GPIO_InitStructure = {0};
    EXTI_InitTypeDef EXTI_InitStructure = {0};
    NVIC_InitTypeDef NVIC_InitStructure = {0};
...
void EXTI0_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(EXTI_Line0) != RESET)
	{
		...
		if( !BSP_ReadPin(POWER_AUTO_CONTROL_PORT,POWER_AUTO_CONTROL_PIN) ){
			OSSemPost( (OS_SEM *)&g_PowerOnsem,OS_OPT_POST_NO_SCHED,&err );               /*释放信号量 关机/开机 */
		}
	}
	OSIntExit();                                  /* EXIT ISR     */ 
}
...
void EXTI2_IRQHandler(void)
{
    ...
    if(EXTI_GetITStatus(EXTI_Line2) != RESET)
    {
		...
		if( !BSP_ReadPin(MOTOR_RESET_PORT,MOTOR_RESET_PIN) ){    /* 只是下降沿发送信号量 */		
			//OSMutexPost( gp_MotorReset );  //
		}else{
			//OSMutexPost( gp_MotorReset );  //
		}
	}
...
void EXTI9_5_IRQHandler(void)
{
    ...
    if(EXTI_GetITStatus(EXTI_Line8) != RESET)
    {
		...
		if( !BSP_ReadPin(ID_DETECT_PORT,ID_DETECT_PIN) ){    /* 只是下降沿发送信号量 */
