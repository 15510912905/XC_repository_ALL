...
/* 初始化SPI接口
  ...
  返回值：成功true，失败false
*/
bool BSP_InitSpiMaster16Bit( BSP_SPI_ENUM spi_number, \
                  ...
                  BSP_PORT_ENUM mosi_port_number, BSP_PIN_ENUM mosi_pin_number )
{
	...
	GPIO_SetBits(PORT(nss_port_number),PIN(nss_pin_number));     /* 必须初始化为高电平 */
	...
	GPIO_SetBits(PORT(sck_port_number),PIN(sck_pin_number));     /* 必须初始化为高电平 */
	...
	GPIO_SetBits(PORT(miso_port_number),PIN(miso_pin_number));     /* 必须初始化为高电平 */
	...
	GPIO_SetBits(PORT(mosi_port_number),PIN(mosi_pin_number));     /* 必须初始化为高电平 */
    ...
    SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;							/*时钟极型选择*/
    SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;						/*时钟边沿选择*/
    SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;			                /*NSS软件控制/硬件控制*/
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;	/* 设置波特率 SPI_BaudRatePrescaler_4 */
...
/* 初始化SPI接口
  ...
  返回值：成功true，失败false
*/
bool BSP_InitSpiMaster8Bit( BSP_SPI_ENUM spi_number, \
                            BSP_PORT_ENUM nss_port_number, BSP_PIN_ENUM nss_pin_number, \
                            BSP_PORT_ENUM sck_port_number, BSP_PIN_ENUM sck_pin_number, \
                            BSP_PORT_ENUM miso_port_number, BSP_PIN_ENUM miso_pin_number, \
                            BSP_PORT_ENUM mosi_port_number, BSP_PIN_ENUM mosi_pin_number )
{
    ...
    SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;						/*时钟边沿选择*/
    SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;			                /*NSS软件控制/硬件控制*/
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;	/*设置波特率*/
    SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;	                /*大端/小端模式*/
    SPI_InitStructure.SPI_CRCPolynomial = 7;			                /*CRC校验多项式*/
...
/* SPI接口发送数据
  ...
  返回值：成功true，失败false
*/
uint16_t BSP_SpiReceiveData(BSP_SPI_ENUM spi_number)
{	
	switch(spi_number)
	{
		case _SPI1_:
...
*/
FlagStatus BSP_SpiGetStatus(BSP_SPI_ENUM spi_number, BSP_SPI_STATUS_TYPE_ENUM type)
{	
	switch(type)
	{
		case TXE_STATUS:
			switch(spi_number)
			{
				...
				case _SPI2_:
					return SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE);			
				case _SPI3_:
					return SPI_I2S_GetFlagStatus(SPI3, SPI_I2S_FLAG_TXE);					
				default:
					return RESET;
			}	
		case RXNE_STATUS:
			switch(spi_number)
			{
				...
				case _SPI2_:
					return SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE);			
				case _SPI3_:
