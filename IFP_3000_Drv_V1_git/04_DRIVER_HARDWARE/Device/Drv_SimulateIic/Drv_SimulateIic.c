...
{	
	...
	BSP_Init_Pin(sda_port_number, sda_pin_number, _OUT_PP_);	/*初始化硬件*/
	...
	BSP_Init_Pin(scl_port_number, scl_pin_number, _OUT_PP_);	/*初始化硬件*/
...
{                        
	...
	_Drv_SimulateIicSetSdaOutput( p_unit );
    ...
    {     
		...
		Drv_SimulateIicDelayUs(p_unit, WAIT_RATE_TIME);   
        
		_Drv_SimulateIicSetScl(p_unit, HIGH);
        
		Drv_SimulateIicDelayUs(p_unit, HALF_RATE_TIME);//RATE_TIME);    /* 适当加长 */
...
unsigned char Drv_SimulateIicReadByte(DRV_SIMULATE_IIC_TYPE* p_unit)
{
	...
	_Drv_SimulateIicSetSdaInput( p_unit );
	...
	//Drv_SimulateIicAck( p_unit );     /* ACK */
	//Drv_SimulateIicNack( p_unit );      /* NACK */
...
{	
	...
	Drv_SimulateIicDelayUs(p_unit, RATE_TIME);
...
bool Drv_SimulateIicWaitAck(DRV_SIMULATE_IIC_TYPE* p_unit)
{
    ...
    Drv_SimulateIicDelayUs(p_unit, ACK_RATE_TIME);//RATE_TIME);
...
bool Drv_SimulateIicAck(DRV_SIMULATE_IIC_TYPE* p_unit)
{
	...
	Drv_SimulateIicDelayUs(p_unit, HALF_RATE_TIME);
...
/*
  ...
  返回：SDA引脚状态, HIGH或者LOW
*/
static uint8_t _Drv_SimulateIicSetSdaInput(DRV_SIMULATE_IIC_TYPE* p_unit)
{
	GPIOA->CRH&=0XFFF0FFFF;
	GPIOA->CRH|=8<<16;
//	switch( p_unit->sda_port_number ){
//		case 0:
...
//			break;
//		case 3:
//			{GPIOD->CRH&=0XFFFF0FFF;GPIOD->CRH|=8<<12;}
//			break;
//		case 4:
//			{GPIOE->CRH&=0XFFFF0FFF;GPIOE->CRH|=8<<12;}
//			break;
//		case 5:
//			{GPIOF->CRH&=0XFFFF0FFF;GPIOF->CRH|=8<<12;}
//			break;
//			
//		default:
//			break;
...
static uint8_t _Drv_SimulateIicSetSdaOutput(DRV_SIMULATE_IIC_TYPE* p_unit)
{
	GPIOA->CRH&=0XFFF0FFFF;
	GPIOA->CRH|=3<<16;
//	switch( p_unit->sda_port_number ){
