...
{	
	...
	BSP_WritePin(p_unit->sck_port_number, p_unit->sck_pin_number, 1);   //CPOL = 1;
	return p_unit;
}
...
uint16_t SoftwareSpiReadByte(DRV_SOFTWARE_SPI_TYPE* p_unit)
{
	uint8_t i = 0;
	uint16_t r = 0;
	for(i=0; i<16; i++)
	{
		p_unit->sck_pin_buffer = 0;
		BSP_WritePin(p_unit->sck_port_number,p_unit->sck_pin_number,p_unit->sck_pin_buffer);
		_SoftwareSpiDelayUs(p_unit, 1);
		p_unit->sck_pin_buffer = 1;
		BSP_WritePin(p_unit->sck_port_number,p_unit->sck_pin_number,p_unit->sck_pin_buffer);
		
		r = (r<<1)|BSP_ReadPin(p_unit->sdi_port_number,p_unit->sdi_pin_number);
		//_SoftwareSpiDelayUs(p_unit, 1);
	}
	return r;
}
/* 软件模拟SPI接口模块延时n微秒
  ...
  返回值：true 成功，false 失败
*/
static bool _SoftwareSpiDelayUs(DRV_SOFTWARE_SPI_TYPE* p_unit, uint32_t time)
{    
   unsigned short i = 0; 
