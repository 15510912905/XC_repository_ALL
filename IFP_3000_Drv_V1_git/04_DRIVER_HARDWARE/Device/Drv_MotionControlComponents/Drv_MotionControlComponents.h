...
typedef enum {
    ...
    MCC_LINE_SLOW = 1,			/*匀速*/
    MCC_LINE_LOW = 2,			/*低速曲线*/
    MCC_LINE_HIGH = 3,			/*高速曲线*/
} MCC_LINE_CHOICE_ENUM;

/*组件复位方向枚举*/
typedef enum {
    MCC_RESET_POSITIVE = 0,		/*正方向复位*/
    MCC_RESET_NEGATIVE = 1,		/*反方向复位*/
} MCC_RESET_DIR_ENUM;

/*组件类型枚举*/
typedef enum {
	MCC_A = 0,			/*只有复位信号的直线机构（柱塞泵）*/
	MCC_B = 1,			/*有复位信号和单码齿信号的直线机构*/
	MCC_C = 2,			/*有复位信号和单码齿信号的旋转机构*/
	MCC_D = 3,			/*有复位信号和多码齿信号的旋转机构*/
	MCC_E = 4,			/*有复位信号和单码齿信号、撞击信号的直线机构（针垂直运动）*/
	MCC_F = 5,			/*有复位信号和多码齿信号的无限旋转机构（盘旋转运动）*/
	MCC_G = 6,			/*有复位信号和多码齿信号的直线机构*/
} MCC_TYPE_ENUM;
...
/* 单个组件复位
  ...
  返回值：err, OS报错代码
*/
extern uint8_t MCC_Reset(DRV_MCC_TYPE* p_MCC, uint32_t timeout);
