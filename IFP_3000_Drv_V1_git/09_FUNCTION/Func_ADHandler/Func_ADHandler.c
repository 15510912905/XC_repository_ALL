...
{		
	...
	uTestInformationRef();                                  /* 刷新测试页内容 */
	uTestInformationPrompt();                               /* 提示显示 */
...
uint8_t uTestInformationRef( void )
{
	...
	if( calendar.w_date!=g_projectSerial.rec_day ){
		...
		g_projectSerial.rec_day = calendar.w_date; /* ????μ±?°ìì */		
	}		
	sprintf( (char *)g_projectSerial.serial_id,"%04d%02d%02d%04d",calendar.w_year,calendar.w_month,calendar.w_date,(g_projectSerial.serial_num+1) );    /* á÷??o?μ?±à??1??òè??ú?óá÷??o? */
...
{
#if HINT_BARCODE_CHECK
	if( gp_stuConfigFile->uReserveChar[0] ){
		uConfirmPage( TJC_WARN_PAGE );                                                       /*设置为强制警告页*/
		uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"Check the Barcode !!!" );	             //	
	}
#endif
...
uint8_t uAcquisitionSystem( uint8_t uStandardQuist )
{
	...
	if( OS_ERR_NONE==err ){
		if( QUICK_RESULT_PAGE==uStandardQuist ){
			AcquisitionQuickSystem( uStandardQuist,STANDARD_TEST );                    /* 采集流程控制 *///OSTimeDlyHMSM(0, 0, 1, 0 );
		}else{
...
//	} 
	...
	for( uTempTestTimeSecond=0;uTempTestTimeSecond<=(uTempTestTime*2);uTempTestTimeSecond++ ){	
	...
	#endif		
#endif
		uConfirmPage( TJC_FORCE_EXIT_PAGE );                           /* 确认在强制退出页 用时130ms */
		...
		OSTimeDlyHMSM(0, 0, 0, 365,OS_OPT_TIME_HMSM_STRICT,&err );    /* 循环快一点 130+5+365=500 */
		g_uRecAutoDownTime = 0;                                       /* 此过程不允许自动关机 */	
	...
	AcquisitionQuickSystem( STANDARD_RESULT_PAGE,STANDARD_TEST );                   /* 标准测试 */
...
#if READCARD_DECTECT
	if( (SWITCH_MIC_ON==gp_stuConfigFile->uSwitchOnOff)&&(STANDARD_RESULT_PAGE!=uStandardQuist) ){
		if( BSP_ReadPin(READCARD_DEC_CONTROL_PORT,READCARD_DEC_CONTROL_PIN) ){            /* 未到位是读回来是1 到位读回来是0 */
			OSTimeDlyHMSM(0, 0, 0, 15,OS_OPT_TIME_HMSM_STRICT,&err); 		
			if( BSP_ReadPin(READCARD_DEC_CONTROL_PORT,READCARD_DEC_CONTROL_PIN) ){
				uTjc4832tSetPage( TJC_WARN_PAGE );                                                                  /*设置为强制警告页*/
...
uint8_t uTheCalibrationCurve( uint32_t* pBuffer,uint32_t uSize )
{
	...
	for( i=0;i<uSize-2;i++ ){
		iRecValue = (int32_t)(pBuffer[i])-(int32_t)(pBuffer[i+1]);
...
{
	return ( *(double*)b - *(double*)a );   /*从大到小*/
}

/* 比较函数 */
int cmpfuncintdoublemin(const void * a, const void * b)
{
	return ( *(double*)a - *(double*)b ); /*从小到大*/
}
...
uint8_t uCalculateDensity( UINTE_DATA_TYPE* pstuIdInfo,ALGORITHM_DATA_TYPE* pstuAlg,float fPesponse,uint8_t uChildrenNum )
{
	...
	switch( pstuIdInfo->children_ufitt ){   /* 确定拟合算法 */
		...
		case FOUR_ARGUMENT:
			...
			break;
		case SPLINE:
			pstuAlforithmSpline( pstuIdInfo,pstuAlg,fPesponse,uChildrenNum );	
			break;
...
uint8_t uAutoQuckTest( void )
{
	...
	if( (1<=g_uAutoTestTimes)&&(g_uAutoTestTimes<=9999) ){
		...
		for( i=g_uAutoTestTimes;i>0;i-- ){	
			...
			uRecError = AcquisitionQuickSystem( QUICK_RESULT_PAGE,DEBUG_TEST );                                             /* 执行快速测试 */
...
#else
	sprintf( (char *)strff,"Ad %d:%d ",iRecFluoresMaxIndex,uRecFluoresMax );
#endif
	uConfirmPage( TJC_AUTOTEST_PAGE );                                                                            /* 确定自动测试页 */
