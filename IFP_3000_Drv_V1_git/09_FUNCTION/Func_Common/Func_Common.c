...
uint8_t uConfirmPage( uint8_t uPage )
{
	...
	iRecPageIDTemp = uTjc4832tGetPage();                    /* 获取当前页面ID 用时120ms */	
	if( EXE_SPECIAL_FAILED!=iRecPageIDTemp ){
		return uConfirmSetPage( uPage,iRecPageIDTemp );     /* 一般用时10ms */
	}else{
...
uint8_t uConfirmSetPage( uint8_t uPage,int16_t iGetPage )
{
	...
	if( uPage!=iGetPage ){
		...
		OSTimeDlyHMSM(0, 0, 0, 10,OS_OPT_TIME_HMSM_STRICT,&err);	             /* 最好不要小于10 */
		return EXE_SUCCEED;	
	}
}
...
uint8_t uConfigAllFileInit( void )
{
	...
	if( 0xff==gp_stuConfigFile->uReserveChar[0] ){
		gp_stuConfigFile->uReserveChar[0] = CURVE_ON;
	}
...
uint8_t uIdDisOver( void )
{
	uReadStoreIdItem( g_uRecordIdCardCover );
	uTestInformation();
...
uint8_t uConfigCurveFile( void )
{	
	if( CURVE_ON==gp_stuConfigFile->uReserveChar[0] ){
		gp_stuConfigFile->uReserveChar[0] = CURVE_ON;
			
		uPromptParameter( (uint8_t *)"Tips on" );
	}else if( CURVE_OFF==gp_stuConfigFile->uReserveChar[0] ){
		gp_stuConfigFile->uReserveChar[0] = CURVE_OFF;	
		
		uPromptParameter( (uint8_t *)"Tips Off" );
	}else{
		gp_stuConfigFile->uReserveChar[0] = CURVE_OFF;
	...
	if( EXE_SUCCEED==SPI_Flash_Write_Page( (u8 *)gp_stuConfigFile,SPIFLASH_CONFIG_FILE_BASE_ADDR,sizeof(CONFIG_FILE) ) ){  /* clear */	
		...
		uPromptParameter( (uint8_t *)"Tips Error" );
		return EXE_FAILED;
	}
}
...
{	
	if( SWITCH_MIC_ON==gp_stuConfigFile->uSwitchOnOff ){
		...
		uPromptParameter( (uint8_t *)"Switch off" );
	}else{
		gp_stuConfigFile->uSwitchOnOff = SWITCH_MIC_ON;
...
uint8_t uSetSystemEditInit( CONFIG_FILE* pstuConfig )
{
	...
	if( (strlen((const char *)strff)>0)&&(0xff!=strff[0]) ){
...
uint8_t uDisParameterInit( void )
{
	...
	memset( strff,0,sizeof(strff) );
	sprintf( (char *)strff,"%d",(int )gp_stuConfigFile->uReserveChar[0] );
	uTjc4832tSetSwitchValue( TJC_INSTRUMENT_PAGE,INSTR_BT2_ID,strff );
...
{	
	...
	if( (0<uStrNum) ){
		...
		if( EXE_SUCCEED==SPI_Flash_Write_Page( (u8 *)gp_stuConfigFile,SPIFLASH_CONFIG_FILE_BASE_ADDR,sizeof(CONFIG_FILE) ) ){  /* clear */	
			if( UI_Y_PROMPT==uPrompter ){
				uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"MF Ok" );		
			}	
#if DEBUH_UART1		
			...
			if( UI_Y_PROMPT==uPrompter ){
				uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"MF Error" );	
			}	
			return EXE_FAILED;
		}
	}else{
