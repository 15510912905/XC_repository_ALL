...
uint8_t uWifiInit( void )
{
	...
	uTempErr += uWifiEnterMode( WIFI_STATION_MODE );                                                                /* 进入sta模式 */
...
//		uRecError += uWifiTransfer( WIFI_TRANSFER_ENTER );          		                               /* 进入透传模式 */
		...
		uRecError = uWifiResetInit();                                                                      /* 恢复出厂设置 */
...
//	uWifiAskMode();                                                                                        /* 模式查询 */
	...
	if( EXE_SUCCEED==uRecError ){
		...
		uRecError = uWifiResetInit();                                                                      /* 恢复出厂设置 */
		OSTimeDlyHMSM(0, 0, 0, 500,OS_OPT_TIME_HMSM_STRICT,&err);	
		uRecError = uWifiInit();                                                                           /* 初始化WIFI */
		uRecError = uClassicBluetoothInit();                                                               /* 经典蓝牙初始化 */
		uTjc4832tSetPage( TJC_WARN_PAGE );                                                                 /* 设置为强制警告页 */
		uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"WIFI Invalid" );	                   /* 无效的WIFI数据 */
	}
	uTjc4832tTouchEnable( TJC_TOUCH_ON );                                                                  /* 整页打开触摸 */
...
uint8_t uWifiConfigServerIPPort( uint8_t* puServerAddr, uint8_t* puPort  )
{
	...
	int32_t iIport = 0;
	uint8_t uRecError = 0;
	uint8_t uRecIpPort = EXE_SUCCEED;
...
//	OSTimeDlyHMSM(0, 0, 1, 0,OS_OPT_TIME_HMSM_STRICT,&err);                                                /* 等待Wifi启动 */
	...
	if( !isVaildIp( (const char *)puServerAddr ) ){                                                        /* IP地址非法判断 */
		uTjc4832tSetPage( TJC_WARN_PAGE );                                                                 /* 设置为强制警告页 */
		uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"IP Invalid" );	                   /* 无效的WIFI数据 */
		uRecIpPort = EXE_FAILED;
	}
	
	uTjcAsciiToInt( puPort,&iIport,strlen((const char *)puPort) );                                         /* IP端口号非法判断 */
	if( (iIport>IPPORT_MAX)&&(iIport<0) ){
		uTjc4832tSetPage( TJC_WARN_PAGE );                                                                 /* 设置为强制警告页 */
		uTjc4832tSetLableValue( TJC_WARN_PAGE,WARN_ID_NUM,(uint8_t *)"Port Invalid" );	                   /* 无效的WIFI数据 */
		uRecIpPort = EXE_FAILED;
	}	
	
	if( EXE_SUCCEED==uRecIpPort ){
...
uint8_t uWifiScanAll( void )
{
	...
	if( WIFI_OFF==gp_stuConfigFile->uWifiOnOff ){
		...
		uErr = uWifiInit();
		uErr = uClassicBluetoothInit();                                                               /* 经典蓝牙初始化 */
	}
...
//		gp_stuConfigFile->uWifiOnOff = WIFI_ON;
//		uWifiOpenClose( WIFI_W01_E103_ON );                                      /* 打开WIFI */
//		OSTimeDlyHMSM(0, 0, 2, 0,OS_OPT_TIME_HMSM_STRICT,&err);                /* 等待Wifi启动 */	
//	}
//	
//	uWifiTransfer( WIFI_TRANSFER_EXIT );                                         /* 退出透彻模式并进入普通模式 CIPMODE */
//	uWifiEnterMode( WIFI_STATION_MODE );                                         /* 进入sta模式 CWMODE */
...
uint8_t uTjcDiaplaySsidName( uint8_t (*upSsidName)[STR_NAME_MAX] )
{
	...
	for( i=0;i<WIFI_AP_MAX;i++ ){
		uTjc4832tSetButterValue( TJC_WIFI_SSID_PAGE,WIFI_BUTTER_ID_1NUM*i,(uint8_t *)"" );
	}
	
//	uConfirmPage( TJC_WIFI_SSID_PAGE );
	
	for( i=0;i<WIFI_AP_MAX;i++ ){
		if( 0!=(*puTempSsidName)[0] ){
			uTjc4832tSetButterValue( TJC_WIFI_SSID_PAGE,WIFI_BUTTER_ID_1NUM*i,*(puTempSsidName++) );
		}
	}
